...
def qry_spread_calculate(**kwargs) -> list:
    """
    用料明細展算查詢
    """
    ...
    cmd: str = f"""SELECT 
        order_key,
        CONVERT(VARCHAR(10), complete_time, 120) AS complete_time,
...
def qry_order_key_by_qry() -> list:
    ...
    """
    msq = XinTeaSql()
    cmd = """SELECT DISTINCT order_key FROM spread_calculate
             ORDER BY order_key"""
...
def qry_m_item_by_qry() -> list:
    ...
    """
    msq = XinTeaSql()
    cmd = """SELECT DISTINCT m_item FROM spread_calculate
             ORDER BY m_item"""
    return msq.s_qry(cmd)
...
def qry_s_item_by_qry() -> list:
    ...
    """
    msq = XinTeaSql()
    cmd = """SELECT DISTINCT s_item FROM spread_calculate
             ORDER BY s_item"""
    return msq.s_qry(cmd)
...
def qry_generate_vendor_by_qry() -> list:
    ...
    """
    msq = XinTeaSql()
    cmd = """SELECT DISTINCT generate_vendor FROM spread_calculate
...
def qry_generate_status_by_qry() -> list:
    ...
    """
    msq = XinTeaSql()
    cmd = """SELECT DISTINCT generate_status FROM spread_calculate
